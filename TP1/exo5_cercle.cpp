...
int main(int argc, char** argv) {
   ...
   // Création de plusieurs VBO
	...
	//Vertex2DColor Intermediaire;
	int N=32;
	float theta = 0;
	float thetaDelta=2*glm::pi<float>()/N;
	float x,y;
	float r=0.5;
	...
	for(int i=0;i<=N;i++){
		x=r*glm::cos(theta);
		y=r*glm::sin(theta);
		vertices.push_back(Vertex2DColor(glm::vec2(x, y), glm::vec3(1, 0, 0)));
		x=r*glm::cos(thetaDelta+theta);
		y=r*glm::sin(thetaDelta+theta);
		vertices.push_back(Vertex2DColor(glm::vec2(x, y), glm::vec3(0, 1, 0)));
		vertices.push_back(Vertex2DColor(glm::vec2(0, 0), glm::vec3(0, 0, 1)));
		theta=thetaDelta+theta;
	}
	/*Vertex2DColor vertices[] = {
		Vertex2DColor(glm::vec2(0, -0.5), glm::vec3(1, 0, 0)),
		Vertex2DColor(glm::vec2(0.5, 0), glm::vec3(0, 1, 0)),
		Vertex2DColor(glm::vec2(0, 0), glm::vec3(0, 0, 1)),
		...
		Vertex2DColor(glm::vec2(-0.5, 0), glm::vec3(1, 0, 0)),
		Vertex2DColor(glm::vec2(0, 0.5), glm::vec3(0, 1, 0)),
		Vertex2DColor(glm::vec2(0, 0), glm::vec3(0, 0, 1)),
	...
	glBindVertexArray(*vaos);
	glDrawArrays(GL_TRIANGLES, 0, 3*N); // Si on veut dessiner plus de triangle le dernier paramétre serait plus grand
	glBindVertexArray(0);
