...
// ------------------------------------------
// Cas ou on veut essayer de calculer dans le shader
uniform float uTime;
...
void main() {
    ...
    if (uTime > 0) {   // calculer avec uTime
      mat3 modelMatrix = rotate(uTime);
      transformed = (vec3(aVertexPosition, 1)*modelMatrix).xy;
    }
    else {    // calculer avec matrice recu de l'appli
      transformed = (uModelTrans*vec3(aVertexPosition, 1)*uModelMatrix).xy;
